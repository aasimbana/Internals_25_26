...
class DbBackupConfigure(models.Model):
    ...
    dropbox_client_key = fields.Char(copy=False, help="Client id of the dropbox")
    dropbox_client_secret = fields.Char(
        copy=False, help="Client secret id of the dropbox"
    )
    ...
    onedrive_client_key = fields.Char(copy=False, help="Client ID of the onedrive")
    ...
    onedrive_folder_key = fields.Char(help="Folder id of the onedrive")
                ...
                self.write(
                    {
                        ...
                        "onedrive_token_validity": fields.Datetime.now()
    ...
    def generate_gdrive_refresh_token(self):
        ...
        try:
            res = requests.post(
                GOOGLE_TOKEN_ENDPOINT, data=data, headers=headers, timeout=20
            )
            res.raise_for_status()
            response = res.content and res.json() or {}
            if response:
                ...
                self.write(
                    {
                        ...
                        "gdrive_token_validity": fields.Datetime.now()
                        + timedelta(seconds=expires_in)
                        if expires_in
                        else False,
                    }
    ...
    def get_gdrive_tokens(self, authorize_code):
        ...
        try:
            res = requests.post(
                GOOGLE_TOKEN_ENDPOINT, params=data, headers=headers, timeout=20
            )
            res.raise_for_status()
            response = res.content and res.json() or {}
            if response:
        ...
        for rec in records:
            ...
            # Onedrive Backup
                ...
                upload_session_url = (
                    ...
                    + "/v1.0/me/drive/items/%s:/%s:/createUploadSession"
                    % (rec.onedrive_folder_key, backup_filename)
                )
                try:
                    upload_session = requests.post(
                        upload_session_url, headers=headers, timeout=20
                    )
                    upload_url = upload_session.json().get("uploadUrl")
                    requests.put(upload_url, data=temp.read(), timeout=20)
                    if rec.auto_remove:
                        list_url = (
                            MICROSOFT_GRAPH_END_POINT
                            + "/v1.0/me/drive/items/%s/children"
                    ...
                    if (
                        ...
                        try:
                            ...
                            ncx = NextCloud(
                                rec.domain,
                                auth=HTTPBasicAuth(
                                    rec.next_cloud_user_name, rec.next_cloud_password
                                ),
                            )
                    ...
                    try:
                        ...
                        if rec.auto_remove:
                            ...
                            response = bo3.list_objects(
                                Bucket=rec.bucket_file_name, Prefix=folder_path
                            )
                            ...
                            for file in response["Contents"]:
                                ...
                                if age_in_days >= rec.days_to_remove:
                                    bo3.delete_object(
                                        Bucket=rec.bucket_file_name, Key=file_path
                                    )
                        ...
                        if rec.aws_folder_name in prefixes:
                            ...
                            s3.Object(
                                rec.bucket_file_name, remote_file_path
