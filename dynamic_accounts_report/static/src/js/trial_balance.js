...
class TrialBalance extends owl.Component {
    ...
    async applyFilter(val, ev, is_delete) {
        ...
        if (
            ev &&
            ev.target &&
            ev.target.attributes["data-value"] &&
        ...
        if (ev) {
            if (
                ev.input &&
                ev.input.attributes.placeholder.value == "Account" &&
                !is_delete
            ) {
                ...
                this.state.selected_analytic =
                    this.state.selected_analytic_account_rec.map((rec) => rec.id);
            }
        } else {
            if (val && val.target.name === "start_date") {
                this.state.date_viewed = [];
                this.state.date_viewed.push(
                    "From" +
                        " " +
                        this.formatDate(this.start_date.el.value) +
                        " " +
                        "To" +
                        " " +
                        this.formatDate(this.end_date.el.value)
                );
                ...
                this.start_date.el.value = today
                    .startOf("month")
                    .toFormat("yyyy-MM-dd");
                ...
                val &&
                val.target.attributes["data-value"].value == "last-month"
            ) {
                this.start_date.el.value = today
                    .startOf("month")
                    .minus({days: 1})
                    .startOf("month")
                    .toFormat("yyyy-MM-dd");
                this.end_date.el.value = today
                    .startOf("month")
                    .minus({days: 1})
                    .toFormat("yyyy-MM-dd");
                this.state.date_viewed = [];
                this.state.date_viewed.push(
                    today.startOf("month").minus({days: 1}).monthShort +
                        " " +
                        today.startOf("month").minus({days: 1}).c.year
                );
                ...
                val &&
                val.target.attributes["data-value"].value == "last-year"
            ) {
                this.start_date.el.value = today
                    .startOf("year")
                    .minus({days: 1})
                ...
                this.state.date_viewed = [];
                this.state.date_viewed.push(
                    today.startOf("year").minus({days: 1}).c.year
                );
                ...
                val &&
                val.target.attributes["data-value"].value == "last-quarter"
            ) {
                this.start_date.el.value = today
                    .startOf("quarter")
                    .minus({days: 1})
                ...
                this.state.date_viewed = [];
                this.state.date_viewed.push(
                    "Q" + " " + today.startOf("quarter").minus({days: 1}).quarter
                );
